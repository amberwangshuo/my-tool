...
**《楚河》The Chu River**（2026-08，甲方 amber，本仓库 `themes/楚河-TheChuRiver.json`，预览页同目录）
- 概念：聊天=隔河对话，双声部结构。甲方给两句词起意：「我欲成冰再也无退路」+「你是我梁山下的楚河 无舟难自渡 亦是我楚辞里的汉歌 无解不疯魔」，交付中甲方补了第三句「你应是有东流的宿命，就别在此结冰」形成对答。char 楼签名行挂「东流」句（冰蓝/劝渡），user 楼挂「成冰」句（朱砂/殉道）；char 卡头冰情观测词七楼轮转（初冰→岸冰→封冻→清沟→冰桥→凌汛→成冰，河不解冻），user 卡头=「过河卒」；元数据="渡No.X·水深X字·渡时Xs"，思维链=『冰层之下』，输入框打字机「仍要渡么——」对发送按钮一个「渡」字，重roll="无舟/仍渡"，加载更多="已渡之河·唯此可回头"（反俄耳浦斯：全界面唯一允许回头处），编辑/更多="改道/支流"，hr=碎冰浮排永远向东流动画
- 视觉：夜#0B0F16+半透明冰面卡片，文源宋体(415)，冰蓝#9FBDCF+朱砂#B0554C 两色纪律，SVG冰裂纹卡头带，:root 预留 --卡头图/--卡头高度 供以后换真图（"先SVG后换图"路线）
- v2 返工教训（重要）：v1 因复用《不可译》骨架被甲方严厉打回（"完全照抄上次，位置一点儿都不变"+"全是文字没高级美化"+"顶栏不能透明"）。v2 重做：Grid区域布局、大卡头图、图上渐变大字(background-clip:text)、反色时间戳章、相框方头像半嵌图下缘、翻页居中、顶栏实底磨砂
- v3 迭代（终版方向）：甲方半途推翻叙事——"完全放弃下棋、渡河元素，只要现代/忧郁/酸涩，切记与流水、冰有关"，并亲自供图4张(catbox)。四图入位：黑浪=char卡头、浮冰=user卡头、白浪=输入区背景、透明底河流剪影=竖贯聊天屏背后(#sheld::before)。配色随图全冷：水青#8FB5AD+冰银#AEB3C1，朱砂降级为仅警示用。文案改写：签名双句保留，大字char=水情词轮转(初冰流凌岸冰封冻清沟冰桥凌汛)/user=成冰，翻页=倒流/东流，输入="趁还没结冰——"+发送章「流」，加载更多="上游·已结冰的部分"，头像加大56px方形
- 甲方新增画像：反感古风小生味（意象可古，气质必须现代）；**讨厌金色**；**每一作必须换全新卡片布局，绝不复用前作骨架**；选"图片框架流"=真的要大图位，且会自己供图（催她给图床直链即可）；叙事隐喻不要具象行为化（棋/渡＝太具象被砍），要情绪化抽象（忧郁/酸涩）；顶栏要实底；头像偏好大方形；会自己丢歌词/诗句当灵感，喜欢设计师替她把叙事想透再拍板；吃"双声部对话"这类文学结构解读
