...
- v2 返工教训（重要）：v1 因复用《不可译》骨架被甲方严厉打回（"完全照抄上次，位置一点儿都不变"+"全是文字没高级美化"+"顶栏不能透明"）。v2 重做：Grid区域布局、140px CSS自绘占位卡头图（--char/user卡头图 变量待甲方填图床直链）、图上渐变大字(background-clip:text)、反色时间戳章、相框方头像半嵌图下缘、翻页居中、朱砂方章「渡」发送键、顶栏实底磨砂
- 甲方新增画像：反感古风小生味（意象可古，气质必须现代）；**讨厌金色**；**每一作必须换全新卡片布局，绝不复用前作骨架**；选"图片框架流"=真的要大图位（先占位后换图），纯文字叙事撑不起她要的华丽度；顶栏要实底；会自己丢歌词/诗句当灵感，喜欢设计师替她把叙事想透再拍板；吃"双声部对话""殉道者/反俄耳浦斯"这类文学结构解读
